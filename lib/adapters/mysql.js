...
module.exports.down() {
  ...
  migrate() {
    return new Promise((resolve, reject) => {
      ...
      if (!thisObj.fs.existsSync(thisObj.migrationsDir)) {
        reject('Migrations folder not found. Please run `db-migrations install` before trying this!');
      }

      const connection = thisObj.mysql.createConnection();
      ...
      connection.query('SELECT name FROM migrations ORDER BY id DESC LIMIT 1', function (error, results) {
        ...
        if (id === 0) {
          ...
          migrationsFiles = thisObj.fs.readdirSync(thisObj.migrationsDir);
        ...
        connection.beginTransaction((err) => {
          migrationsFiles.forEach((migrationFile) => {
            if (!thisObj.fs.existsSync(`${thisObj.migrationsDir}/${migrationFile}`)) {
              reject(`Ops! Something went wrong while trying to find ${thisObj.migrationsDir}/${migrationFile}`);
            }

            const migration = require(`${thisObj.migrationsDir}/${migrationFile}`);

            console.log(`Running ${thisObj.migrationsDir}/${migrationFile} migration`);
            ...
            connection.query('INSERT INTO migrations SET name = ?', migrationFile, (error, results, fields) => {
              ...
              console.log(`Migration ${thisObj.migrationsDir}/${migrationFile} executed`);
            });
          });
      ...
      connection.query('SELECT id, name FROM migrations ORDER BY id DESC', function (error, results) {
        ...
        connection.beginTransaction((err) => {
          results.forEach((result) => {
            ...
            console.log(`Rolling back #${migrationId} - ${thisObj.migrationsDir}/${migrationFile} migration`);
            ...
            if (migrationId > 1) { // the first one executed is the migrations table creation itself, so in rollback it'll delete the table
              connection.query('DELETE FROM migrations WHERE id = ?', migrationId, (error, results, fields) => {
                ...
                console.log(`Migration #${migrationId} - ${thisObj.migrationsDir}/${migrationFile} rolled back`);
              });
            }
          });
